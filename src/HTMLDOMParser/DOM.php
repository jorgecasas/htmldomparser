...
/**
 * hmldomparser.php - Class for parsing HTML DOM
 ...
 */
/**
 * Class DOM for accessing the DOM using PHP.
 * Based on simple_html_dom
 ...
 *
 * @return false|\HTMLDOMParser\DOM
 */
function file_get_html( $url, $use_include_path = false, $context, $offset = -1, $maxLen = -1, $lowercase = true, $forceTagsClosed = true, $target_charset, $stripRN = true, $defaultBRText, $defaultSpanText ) {
...
class DOM_Node {
    ...
    /**
     * Clear memory (if not, creates circular reference)
     */
    ...
    /**
     * Debugging public function to dump a single dom node with a bunch of information about it.
     *
     * @param bool $echo Screen echo for debuging
     *
     * @return string
     */
     ...
     * @return mixed
     */
    public function get_parent( $parent = null ) {
        if ( $parent !== null ) {
            $this->parent = $parent;
    ...
    // public function to locate a specific ancestor tag in the path to the root.
    ...
    public function find( $selector, $idx = null, $lowercase = false ) {
        ...
        // find each selector
        foreach ( $selectors as $cValue ) {
            // The change on the below line was documented on the sourceforge code tracker id 2788009
            // used to be: if (($levle=count($selectors[0]))===0) return array();
            if ( ( $levle = count( $cValue ) ) === 0 ) {
                return array();
            }
    ...
    // - added parameter to allow for case insensitive testing of the value of a selector.
    protected function seek( $selector, &$ret, $lowercase = false ) {
        ...
        for ( $i = $this->_[ HDOM_INFO_BEGIN ] + 1; $i < $end; ++$i ) {
            ...
            if ( $pass && $key ) {
                ...
                } elseif ( ( $key != "plaintext" ) && !isset( $node->attr[ $key ] ) ) {
                    $pass = false;
                }
            }
            // compare value
        ...
        // It's passed by reference so this is actually what this public function returns.
        if ( is_object( $debug_object ) ) {
    ...
    public function convert_text( $text ) {
        ...
        if ( $targetCharset == 'UTF-8' ) {
            if ( strpos( $converted_text, "\xef\xbb\xbf" ) === 0 ) {
    ...
    public function get_display_size() {
        ...
        // See if there is height or width attribute in the tag itself.
        ...
        if ( isset( $this->attr[ 'style' ] ) ) {
            ...
            // If there is width in the style attributes:
            // check that the last two characters are px (pixels)
...
class DOM {
    ...
    public function load( $str, $lowercase = true, $stripRN = true, $defaultBRText = DEFAULT_BR_TEXT, $defaultSpanText = DEFAULT_SPAN_TEXT ) {
        ...
        // make load public function chainable
        return $this;
    }
    ...
    /**
     * Returns HTML string from DOM objects
     * @return string HTML
     */
    ...
    //  - dkchou - added this to try to identify the character set of the page we have just parsed so we know better how to spit it out later.
    ...
    protected function parse_charset() {
        ...
        $charset_strtolower = strtolower( $charset );
        if ( $charset_strtolower === 'iso-8859-1' || $charset_strtolower === 'latin1' || $charset_strtolower === 'latin-1' ) {
            $charset = 'CP1252';
        }
    ...
    protected function read_tag() {
        ...
        if ( $this->char === '/' ) {
            $this->char = ( ++$this->pos < $this->size ) ? $this->doc[ $this->pos ] : null; // next
            // This represents the change in the DOM trunk from revision 180 to 181.
    ...
    protected function parse_attr( $node, $name, &$space ) {
        ...
        //  If this is a "class" selector, lets get rid of the preceeding and trailing space since some people leave it in the multi class case.
        if ( $name == "class" ) {
    ...
    public function restore_noise( $text ) {
        ...
        while ( ( $pos = strpos( $text, '___noise___' ) ) !== false ) {
            ...
            if ( strlen( $text ) > $pos + 15 ) {
                ...
                } else { // do this to prevent an infinite loop.
                    $text = substr( $text, 0, $pos ) . 'UNDEFINED NOISE FOR KEY: ' . $key . substr( $text, $pos + 16 );
                }
            } else { // There is no valid key being given back to us... We must get rid of the ___noise___ or we will have a problem.
